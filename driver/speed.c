...
static int irql;
static int irqr;
static int Right_Wheel(void){
	if(gpio_get_value(10)==1){
		message[0]--;
	}else{
		message[0]++;
	}
...
static int Left_Wheel(void){
	if(gpio_get_value(8)==1){
		message[1]--;
	}else{
		message[1]++;
	}
...
/////////////////////////////////////////////////////////////////////////
	...
	gpio_request(10, "right");
	gpio_direction_input(10);

	disable_irq(gpio_to_irq(7));
	enable_irq(gpio_to_irq(7));
	disable_irq(gpio_to_irq(9));
	enable_irq(gpio_to_irq(9));

	if(request_irq(gpio_to_irq(7), (irq_handler_t)Left_Wheel, IRQF_TRIGGER_FALLING, "left", &irql)) return 1;
	if(request_irq(gpio_to_irq(9), (irq_handler_t)Right_Wheel, IRQF_TRIGGER_FALLING, "right", &irqr)) return 1;
