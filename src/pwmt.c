...
#define  DevAddr  0x68 //53  //device address
...
void AD_calculate(void)
{
	...
	acc=(32768-aaa*1.0)/16384;//+(2048-Get_Adc(0))*0.0005;//在这角度调零
	if(acc>1)                  ///这个地方必须限幅，不然会出问题
	acc=1;
	else if(acc<-1)
	acc=-1;
	acc=57.3*asin(acc);			//计算出角度
	ggg=mpu6050_buffer[8]*256+mpu6050_buffer[9]+32768;
	gyro=(32768-ggg*1.0)/131;	//计算出加速度
...
{  	
	...
	position_dot_filter+=position_dot*0.15;	//车轮速度滤波
	
	position+=position_dot_filter; //积分得到位移
	position+=Speed_Need;
	if(position<-800)		//防止位置误差过大导致的不稳定
	{
		position=-800;
	}
...
//    add_channel_pulse(channel, 4, 0, 50);
//    add_channel_pulse(channel, gpio, 100, 50);
//    add_channel_pulse(channel, gpio, 200, 50);
//    add_channel_pulse(channel, gpio, 300, 50);
//    usleep(demo_timeout);    add_channel_pulse(channel, 17, 0, 150);

//    usleep(demo_timeout);    add_channel_pulse(channel, 18, 0, 250);

//    usleep(demo_timeout);    add_channel_pulse(channel, 22, 0, 200);

//    usleep(demo_timeout);    add_channel_pulse(channel, 23, 0, 100);
//    usleep(demo_timeout);
...
for(;;){
		...
		add_channel_pulse(channel, 18, 0, 1.0*acc_xyz.gx/65535*2000);
		printf("x: %05d  y: %05d  z: %05d gx: %05d  gy: %05d  gz: %05d\n", acc_xyz.x, acc_xyz.y, acc_xyz.z, acc_xyz.gx, acc_xyz.gy, acc_xyz.gz);
